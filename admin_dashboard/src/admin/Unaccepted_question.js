...
function Unaccepted_question(props) {
  ...
  useEffect(()=>{
    ...
    function getauction(){
    axios.get(`http://localhost/project_9/admin_dashboard/API/admin_question.php?is_accepted=0`)
      // console.log(aa)
      .then((res) => {
        //  console.log(res)
        const info = res.data;
  ...
  const accept_question=(postId)=>{
    // axios.delete(`http://localhost/redux-project9/API/deleteauction.php${postId}/delete`)
    axios.delete('http://localhost/project_9/admin_dashboard/API/accep_question.php?id='+postId)
    .then(function(res){
      console.log(res.data);
      const info = res.data;
        getauction(info);
        ...
        <table>
          ...
          <tbody>
            ...
            <tr className='td-data' key={info}>
              ...
              <td>
        <button className="btn" onClick={()=>accept_question(info.question_id)}> Accept</button>
        {/* <Link to={`/EditAuctionAdmin/${info.id}/edit`} className="btn" style={{borderRadius:'12px'}}>Edit</Link> */}
        <button className="btn btn-delete" onClick={()=>delete_question(info.question_id)}>Deny</button>
...
export default Unaccepted_question;